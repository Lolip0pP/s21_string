...
setup_t get_setup(int *cur, const char *format, va_list *params) {
  ...
  for (; format[*cur] == '*'; *cur += 1) setup.width = va_arg(*params, int);
...
int read_specifier(char *result, char spec, setup_t setup, va_list *params) {
  ...
  if ((!(setup.flags & F_L_ALIGNMENT) && setup.flags & F_ZERO) &&
      (setup.flags & F_FLOAT || s21_strchr("cs%", spec)))
    length = format(
        result +
            (s21_strchr("+- ", *result) && !s21_strchr("sc", spec) ? 1 : 0),
        length, setup.width, setup.flags, '0');
...
long double get_double(va_list *params, int type) {
  return type & F_LONG_D ? va_arg(*params, long double)
...
int read_arg(char *str, setup_t setup, long double arg) {
  ...
  if (setup.flags & F_FLOAT)
    r_arg = roundl(
        arg * pow(10, setup.accuracy - log - (setup.flags & F_GEN ? 1 : 0)));
  ...
  if (!(setup.flags & (F_FLOAT | F_ACCURACY)) && setup.flags & F_ZERO)
    setup.accuracy =
        setup.width - (setup.flags & (F_MINUS | F_PLUS | F_SPACE) ? 1 : 0);

  num_to_string(str, r_arg, &length, setup.accuracy, setup.flags);

  if (setup.flags & F_MANTISSA) length += mantissa(str + length, setup, log);

  return length;
}

void num_to_string(char *str, unsigned long int num, int *index, int accuracy,
                   unsigned int flags) {
  ...
  if (num > 0)
    ...
    if (flags & F_HEX && flags & F_POINT)
      str[(*index)++] = '0', str[(*index)++] = flags & F_UPP ? 'X' : 'x',
      accuracy -= 2;

    if (flags & F_OCT && flags & F_POINT) str[(*index)++] = '0', accuracy -= 1;

    if (accuracy > 0)
      s21_memset(str + *index, '0', accuracy - 1), *index += accuracy - 1;
  }

  str[(*index)++] = remains + (remains < 10    ? '0'
                               : flags & F_UPP ? 'A' - 10
                                               : 'a' - 10);

  if ((!accuracy && flags & F_FLOAT) && (flags & F_POINT || !(flags & F_START)))
    str[(*index)++] = '.';
}
